...
import { CheckIcon, CopyIcon, TrashIcon, WhatsAppIcon, XIcon } from '@/components/ui/icons';
...
export function AppointmentsPanel({ barbers, services, settings }: AppointmentsPanelProps) {
  ...
  const sortValue = (appointment: Appointment, key: SortKey): string | number => {
    switch (key) {
      ...
      case 'barber':
        return barberName(appointment.barberId);
      case 'service':
        return serviceName(appointment.serviceId);
      case 'payment':
        return appointment.amount ?? -1;
      case 'debt':
  ...
  }, [appointments, sortKey, sortDir, barbers, services]);
    ...
    <section aria-labelledby="admin-appointments-title">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:items-end">
          ...
          <button type="button" onClick={() => load()} className="pill-outline w-full sm:w-auto">
            Actualizar
          </button>
        </div>
      </header>
          ...
          <div className="mt-7 space-y-3 lg:hidden">
            {sortedAppointments.map((appointment, index) => (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-gray-100 pt-3 text-sm sm:grid-cols-3">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.1em] text-ink-muted">
                      Barbero
                    </p>
                    <label className="sr-only" htmlFor={`barber-mobile-${appointment.id}`}>
                      Barbero de {appointment.customerName}
                    </label>
                    <select
                      id={`barber-mobile-${appointment.id}`}
                      value={appointment.barberId}
                      onChange={(event) => reassignBarber(appointment, event.target.value)}
                      className="mt-1 w-full rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-xs text-ink-soft"
                    >
                      {barbers.map((barber) => (
                        <option key={barber.id} value={barber.id}>
                          {barber.name}
